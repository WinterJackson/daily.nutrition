...
import { getMediaFiles, uploadFile } from "@/app/actions/media"
...
export function MediaPickerModal({ open, onOpenChange, onSelect, folder = "daily_nutrition_uploads", allowedTypes = "all" }: MediaPickerModalProps) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        startTransition(async () => {
            try {
                // Xinteck Pattern: Client-side WebP optimization if image
                    ...
                    <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex gap-2 mr-4">
                            <input 
                                ...
                                accept={allowedTypes === "image" ? "image/*" : allowedTypes === "video" ? "video/mp4,video/webm" : "image/*,video/mp4,video/webm"}
                            />
                            <Button onClick={handleUploadClick} disabled={isUploading} size="sm" variant="accent">
                                {isUploading ? (
                                    ...
                                    {filteredFiles.map((file) => {
                                        ...
                                        return (
                                            <div 
                                                key={file.id} 
                                                onClick={() => {
                                                    onSelect(file.url, file.publicId)
                                                    onOpenChange(false)
                                                }}
                                            ...
                                            >
                                                <div className="relative aspect-video bg-neutral-100 dark:bg-white/5 flex items-center justify-center overflow-hidden">
                                                    {isVideo ? (
                                                        ...
                                                        <Image src={file.url} alt={file.filename} fill sizes="200px" className="object-cover group-hover:scale-105 transition-transform duration-300" />
                                                    )}
                                                ...
                                                <div className="p-2 bg-white dark:bg-black/20 flex items-center gap-2 border-t border-subtle">
                                                    {isVideo ? <FileVideo className="w-3 h-3 text-blue-500 shrink-0" /> : <FileImage className="w-3 h-3 text-brand-green shrink-0" />}
                                                    <p className="text-[10px] font-medium truncate flex-1" title={file.filename}>{file.filename}</p>
                                                </div>
                                            </div>
                                        )
                                    })}
