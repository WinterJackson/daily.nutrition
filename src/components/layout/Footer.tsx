      ...
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div className="space-y-4 text-center md:text-left">
            ...
            <ul className="space-y-2 text-sm text-white/80">
              {displayedServices.length > 0 ? (
                displayedServices.filter(service => service.id !== "discovery-call").map(service => (
                    <li key={service.id}>
          ...
          <div className="space-y-4 text-center md:text-left">
            ...
            <h4 className="font-semibold text-lg text-white mt-8 mb-4">Social Links</h4>
            <div className="flex flex-col gap-3 justify-center md:justify-start pt-2">
              {settings.facebookUrl && (
                <Link href={settings.facebookUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-orange hover:text-orange/80 transition-colors">
                  <Facebook className="h-5 w-5" /> <span>Facebook</span>
                </Link>
              )}
              {settings.instagramUrl && (
                <Link href={settings.instagramUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-orange hover:text-orange/80 transition-colors">
                  <Instagram className="h-5 w-5" /> <span>Instagram</span>
                </Link>
              )}
              {settings.twitterUrl && (
                <Link href={settings.twitterUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-orange hover:text-orange/80 transition-colors">
              ...
              {settings.linkedinUrl && (
                <Link href={settings.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-orange hover:text-orange/80 transition-colors">
