...
export function ServicesPageContent({ services }: { services: Service[] }) {
    ...
    return (
        <div className="min-h-screen bg-off-white dark:bg-charcoal pt-36 pb-24 relative overflow-hidden">
            {/* Animated Background */}
            ...
            <section className="relative z-10 container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 text-center">
                ...
                >
                    Comprehensive, evidence-based nutrition programs tailored to
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 min-h-75">
                                ...
                                <div className="md:col-span-5 lg:col-span-6 p-8 md:p-12 text-center md:text-left z-10">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-4 justify-center md:justify-start">
                                        <Button
                                            asChild
                                            size="lg"
                                            className="bg-brand-green hover:bg-brand-green/90 text-white font-semibold px-8 h-12 rounded-full shadow-lg shadow-brand-green/20"
                                        >
                                            <Link
                                                href={`/booking?service=${discoveryService.slug}`}
                                            >
                                                Book Now - Free
                                            </Link>
                                        </Button>
                                        <Button
                                            asChild
                                            variant="ghost"
                                            className="text-olive dark:text-off-white hover:bg-olive/5 dark:hover:bg-white/10"
                                        >
                                            <Link
                                ...
                                <div className="hidden md:block md:col-span-4 lg:col-span-3 relative bg-neutral-100 dark:bg-white/5">
                                    {discoveryService.image ? (
                                        <>
                                            <div className="absolute inset-y-0 left-0 w-24 bg-linear-to-r from-white/60 to-transparent dark:from-charcoal/0 z-10" />
                                            <Image
                    ...
                    {gridServices.map((service, index) => (
                        ...
                        >
                            <Card className="h-full hover:shadow-xl transition-all border-neutral-200/80 dark:border-white/10 overflow-hidden flex flex-col group bg-white/90 dark:bg-charcoal/90 backdrop-blur-sm">
                                <div
                                    className={`h-1 w-full ${service.bgColor.replace("/10", "")} origin-left transform duration-500 group-hover:scale-x-100 scale-x-0`}
                                />
                                    ...
                                    {service.image && (
                                        <div className="w-full h-56 relative">
                                            <Image
                                                ...
                                                className="object-cover"
                                            />
                                            <div className="absolute inset-0 bg-linear-to-t from-white dark:from-charcoal to-transparent opacity-90" />
                                        </div>
                                    )}
                                    <div
                                        className={`px-6 pb-2 relative ${!service.image ? "pt-6" : "-mt-16 z-10"}`}
                                    >
                                        <div
                                            className={`absolute right-4 p-2.5 rounded-xl ${service.bgColor} ${service.color} group-hover:scale-110 transition-transform ${service.image ? "top-0 shadow-md bg-white dark:bg-charcoal" : "top-4 opacity-80"}`}
                                        >
                                            <ServiceIcon
                                                name={service.icon}
                                                className="w-5 h-5"
                                ...
                                <CardContent className="grow pt-0">
                                    ...
                                    <div className="h-px w-full bg-neutral-100 dark:bg-white/5 mb-4" />
                                ...
                                <CardFooter className="pt-2 pb-4">
                                    <Button
                                        asChild
                                        variant="ghost"
                                        className="w-full justify-between hover:bg-transparent hover:text-orange group/btn p-0 h-auto"
                                    >
                                        <Link
