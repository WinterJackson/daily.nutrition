...
export function ServiceSelection({ onServiceSelect, selectedServiceId, activeServiceIds, sessionType, onSessionTypeChange, services = [] }: ServiceSelectionProps) {
    ...
    <div className="space-y-12">
      ...
      {discoveryService && (
        ...
        >
           ...
           <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="flex-shrink-0 z-10 w-full md:w-auto flex justify-center md:justify-start">
              ...
              <div className="flex-grow text-center md:text-left z-10 relative md:pr-48">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {gridServices.map((service) => (
          ...
          >
            {/* Header / Image Area */}
            <div className="relative w-full h-32 mb-4 bg-neutral-100 dark:bg-white/5">
                {service.image ? (
                    <>
                        <Image src={service.image} alt={service.title} fill className="object-cover" />
                        <div className="absolute inset-0 bg-gradient-to-t from-white/90 dark:from-charcoal/90 to-transparent"></div>
                    </>
                ) : (
                    <div className="w-full h-full flex items-center justify-center opacity-10">
                        <ServiceIcon name={service.icon} className={`w-12 h-12 ${service.color}`} />
                    </div>
                )}
                {/* Selection Check */}
            <div className={cn(
            ...
            <div className={cn("absolute -bottom-6 right-6 w-12 h-12 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 shadow-md z-10", service.bgColor, service.color)}>
